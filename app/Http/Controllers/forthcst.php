...
class forthcst extends Controller
{
    public function forthcstd()
    {
      
        return view('/employee/llllcst/forthcst');
    }
    public function forthcstmanual()
    {
      
        return view('/employee/llllcst/forthcstmanual');
    }
    public function forthcstresultmanual(Request $req)
    {

        $data = forthcstmodel::where('roll', '=', $req->roll)->first();
      ...
      if(!$data){
        ...
        return redirect('forthcstmanage');
            }else{

                session()->flash('message', 'Data already have!');

                return redirect('forthcstmanual');

            }
    }
    

    public function forthcstmanage()
    {
        $showdata =forthcstmodel::all();

        $showdatacout = count($showdata);

        return view('employee/llllcst/forthcstmanage', compact('showdata', 'showdatacout'));  
    }
    public function forthcstpdf()
    {
      
        return view('/employee/llllcst/forthcstpdf');
    }
    public function forthcstfees()
    {
      
        $showdata = fees::where('teqnology', '=', 'forthcst')->first();
        return view('employee/llllcst/forthcstfees' ,compact('showdata'));
    }

    public function forthcstfeesi(Request $input)
    {
        $showdata = fees::where('teqnology', '=', 'forthcst')->first();
        if(!$showdata==null){
        ...
        return redirect('/forthcstfees');
    }


    public function forthcstpdfi(Request $req)
    {
        $data=$req->inputText;
        ...
        for ($i = 0; $i < count($rolls); $i++) {
                $forthcst = new forthcstmodel();
                    $forthcst->roll = $rolls[$i];
                    $forthcst->result = $results[$i];
                    $forthcst->save();           
                }
        for ($i = 0; $i < count($rolls_set); $i++) {
            $forthcst = new forthcstmodel();
            $forthcst->roll = $rolls_set[$i];
...
$seventh_nums = rtrim($seventh_nums, ",");
   ...
   if($first_nums){$forthcst->first = $first_nums;};
   if($second_nums){$forthcst->second = $second_nums;};
   if($third_nums){$forthcst->third = $third_nums;};
   if($fourth_nums){$forthcst->fourth = $fourth_nums;};
   if($fifth_nums){$forthcst->fifth = $fifth_nums;};
   if($sixth_nums){$forthcst->sixth = $sixth_nums;};
   if($seventh_nums){$forthcst->seventh = $seventh_nums;};
    ...
    public function edit($roll)
    {
        ...
        $showdata = forthcstmodel::where('roll', '=', $roll)->first();
      
        return view('employee/llllcst/edit',compact('showdata'));
    }

    public function editforthcstresult(Request $req,$roll)
    {
    ...
    public function delletforthcstdata($roll)
    {
